...
\talktitle{Linear algebra methods for Boolean functions}
...
\talktitle{Preconditioning via Randomized Range Deflation (RandRAND)}
...
\talktitle{A data-dependent regularization method based on the graph Laplacian}
...
\mtskip
    Drawing random samples distributed as a multi-variate Gaussian probability distribution is exactly linear algebra (plus standard normal random number generation). The correspondence between Gauss--Seidel iteration and Gibbs sampling, in the sense that the conditions for convergence, iteration operators, error polynomials, and rates of convergence are all identical, provides the basic mathematical and algorithmic link between iterative sampling algorithms and the mature field of iterative methods in linear algebra. In high dimensions, rates of convergence may be carried over to iterative sampling from non-Gaussian distributions. We develop that connection and explore some more advanced sampling algorithms based on advanced numerical linear algebra.
\end{ilasabstract}
...
\talktitle{A combinatorial bound on the number of distinct eigenvalues of a graph}
    
\textbf{H. Tracy Hall}, \info{17:00\textrm{--}17:30 @ SC1005 (June 26, Thursday)} \hfill \hyperlink{up0377}{$\Uparrow$}
...
\textbf{Sooyeong Kim}, \info{11:00\textrm{--}11:30 @ SC0014 (June 25, Wednesday)} \hfill \hyperlink{up0251}{$\Uparrow$}
...
\talktitle{Estimation of noise covariances in Kalman filtering by structure preserving doubling algorithm}
...
\talktitle{Special orthogonal, special unitary, and symplectic groups as products of Grassmannians}
...
\talktitle{Compositional optimization of Rayleigh quotients and related distance problems for matrix polynomials}
...
\talktitle{Phase-isometries on continuous function spaces}
...
\talktitle{A new characterization of the weighted Kubo-Ando geometric means via composite quantum state discrimination}
...
\talktitle{Distinct spectra of integer matrices and closed walk matrices of graphs}
...
\talktitle{Law of large numbers for generalized operator means}
...
\talktitle{Randomized preconditioned solvers for Strong Constraint 4D-Var data assimilation}
...
\talktitle{MinAres and CAr: New Krylov methods for symmetric linear systems}
...
\talktitle{Geometric properties and distance inequalities on Grassmannians}
...
\talktitle{Reverse data-processing inequalities for partial transposes of random quantum states}
...
\talktitle{Blockwise bistochastic matrices and operator majorisation}
    
\textbf{Karol Życzkowski}, \info{14:30\textrm{--}15:00 @ SC0014 (June 23, Monday)} \hfill \hyperlink{up0053}{$\Uparrow$}
